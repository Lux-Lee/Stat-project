...
F_model1<- lm(log(Indigenous)~Year_n*Canadian+Category_n+Gender_n, data=genders_data)
F_model2<- lm(log(Indigenous)~Year_n*Canadian+Category_n, data=genders_data)
F_model3<- lm(log(Canadian)~Year_n*Indigenous+Category_n+Gender_n, data=genders_data)
F_model4<- lm(log(Canadian)~Year_n*Indigenous+Category_n, data=genders_data)
summary(F_model2)
summary(F_model4)
summary(lm(log(Canadian)~Year_n*Indigenous+Category_n, data= subset(genders_data, Category_n == "2")))
anova(F_model2,F_model4)
...
Cov_model <- lm(log(Vaccination)~Gender+Group+Period, data= Gender_data)
summary(Cov_model)
summary (lm(log(Percentage)~Group_n+Gender_n+Category_n+Period, data=Health_Data))